...
export const Card = ({
    ...
    className = "",
    action,
}: PropsWithChildren<{ title?: string; className?: string; action?: any }>) => (
    <div
    ...
    >
        {(title || action) && (
            <div class="flex justify-between items-center mb-5 pb-4 border-b border-gray-100 dark:border-slate-700">
                {title && (
                    <h3 class="text-xl font-bold bg-clip-text text-transparent bg-linear-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300">
                        {title}
                    </h3>
                )}
        ...
        <div class="dark:text-gray-200">{children}</div>
    </div>
);
...
>) => {
    const base =
        "px-4 py-2 rounded-lg font-medium focus:outline-none transition duration-200 shadow-sm transform active:scale-95";
    const variants: Record<string, string> = {
        primary:
            "bg-primary-600 text-white hover:bg-primary-700 dark:bg-primary-600 dark:hover:bg-primary-500 shadow-primary-500/30",
        danger: "bg-red-600 text-white hover:bg-red-700 dark:bg-red-600 dark:hover:bg-red-500 shadow-red-500/30",
        warning:
            "bg-yellow-500 text-white hover:bg-yellow-600 dark:bg-yellow-600 dark:hover:bg-yellow-500 shadow-yellow-500/30",
        secondary: "bg-gray-600 text-white hover:bg-gray-700 dark:bg-slate-600 dark:hover:bg-slate-500",
        outline:
            "border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700",
    };
    return (
...
export const Badge = ({
    children,
    color = "gray",
}: PropsWithChildren<{ color?: "green" | "red" | "blue" | "gray" }>) => {
...
export const Table = ({ headers, children }: { headers: string[]; children: any }) => (
    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-slate-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            ...
            <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-700">{children}</tbody>
        </table>
    </div>
);
